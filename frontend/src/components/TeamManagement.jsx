...
function EmployeeModal({ open, onClose, onSave, editData }) {
  ...
  useEffect(() => {
    setForm(editData ? { ...editData, salary: String(editData.salary) } : DEFAULT_FORM);
    setErrors({});
  }, [editData, open]);
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
          ...
          <button onClick={handleSubmit} disabled={saving}
            className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 shadow-sm transition disabled:opacity-60">
...
export default function TeamManagement() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (!res.ok) { showToast("Failed to delete employee", "error"); return; }
  ...
  const handleEdit = (emp) => { setEditData(emp); setModalOpen(true); };
        ...
        <div className="overflow-x-auto">
          ...
          {!loading && filtered.length === 0 && (
            <div className="text-center py-16 text-gray-400">
              ...
              <p className="text-xs mt-1">Add your first team member using the button above</p>
            </div>
          )}
        </div>
      </div>
