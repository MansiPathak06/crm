  ...
  return (
    <div className="flex items-center gap-2 min-w-[110px]">
      <div className="flex-1 bg-gray-100 rounded-full h-2">
        <div
          className={`h-2 rounded-full ${barColor[status]} transition-all duration-500`}
          style={{ width: `${value}%` }}
        />
      </div>
...
export default function ProjectTable({ projects, onView, onEdit }) {
  return (
    <>
      {/* Desktop Table */}
          ...
          <tbody className="divide-y divide-gray-50">
            {projects.map((p, i) => {
              ...
              return (
                <tr
                  key={p.id}
                  className={`hover:bg-gray-50/70 transition-colors duration-150 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/30"}`}
                >
      ...
      <div className="md:hidden grid grid-cols-1 sm:grid-cols-2 gap-4">
        {projects.map((p) => (
          <ProjectCard key={p.id} project={p} onView={onView} onEdit={onEdit} />
        ))}
        {projects.length === 0 && (
