...
export default function AddProjectModal({ open, onClose, onAdd }) {
  ...
  const handleSubmit = () => {
    ...
    onAdd({ ...form, id: Date.now(), progress: 0, budget: `₹${Number(form.budget).toLocaleString("en-IN")}` });
    setForm(defaultForm);
    setErrors({});
    onClose();
  };
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-teal-500 px-6 py-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-emerald-100 text-sm">Fill in the details to create a project</p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 shadow-sm transition"
          >
            Create Project
          </button>
        </div>
